...
export function generateMongoDBPassword(length = 20): string {
  const safeLength = Math.max(12, length)

  const requiredCharacters = [pickRandom(LOWERCASE), pickRandom(UPPERCASE), pickRandom(NUMBERS), pickRandom(SYMBOLS)]
