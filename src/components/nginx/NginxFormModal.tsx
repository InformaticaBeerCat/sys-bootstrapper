...
function validate(values: NginxServerInput, t: Dictionary): string | null {
  ...
  if (!values.isProxy && !values.path.trim()) return t.nginx.pathRequired
  if (values.isProxy && !values.proxyTarget.trim()) return t.nginx.proxyRequired
...
export function NginxFormModal({ mode, initialValues, onCancel, onSave }: NginxFormModalProps) {
  ...
  return (
    <Modal
      title={mode === 'create' ? t.httpServer.newTitle('Nginx') : t.httpServer.editTitle('Nginx')}
      onClose={onCancel}
      size="lg"
      footer={(requestClose) => (
        <>
    ...
    >
      <div className="form-row form-row-1">
        <div className="form-group">
          <label className="form-label">{t.nginx.domainsInput}</label>
          <input
      ...
      <div className="form-row">
        <div className="form-group">
          <label className="form-label">{t.nginx.pathInput}</label>
          <input
            className="text-input"
            type="text"
