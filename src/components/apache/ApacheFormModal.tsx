...
function validate(values: ApacheServerInput): string | null {
  ...
  if (!values.isProxy && !values.path.trim()) return 'El path del sitio es obligatorio.'
  if (values.isProxy && !values.proxyTarget.trim()) return 'El destino del proxy es obligatorio.'
...
export function ApacheFormModal({ mode, initialValues, onCancel, onSave }: ApacheFormModalProps) {
  ...
  return (
    <Modal
      title={mode === 'create' ? 'Nueva configuración Apache' : 'Editar configuración Apache'}
      onClose={onCancel}
      size="lg"
      footer={(requestClose) => (
        <>
    ...
    >
      ...
      <div className="form-row">
        <div className="form-group">
          <label className="form-label">Path del sitio (DocumentRoot)</label>
          <input
            className="text-input"
            type="text"
